        ...
        records.append(
            {
                ...
                **{key: message[key] for key in ("source_id", "source_floor", "version", "raw_content") if key in message},
            }
        )
    return records
...
def _evidence_quote_in_text(quote: str, text: str) -> bool:
    """Locate the whole quote; never let one true fragment justify an invented suffix."""
...
def validate_entity_plan(
    ...
    for index, entity in enumerate(entities):
        ...
        if existing is None and name and not str(key).endswith(":" + name):
    ...
    while True:
        ...
        for transport_attempt in range(1, transport_attempt_limit + 1):
            try:
                ...
                if transport_attempt == transport_attempt_limit or metadata.get("retryable") is False:
                    attempts.append(
                        {
                ...
                time.sleep(max(float(metadata.get("retry_after", 0)), 2**transport_attempt) + max(0.0, retry_offset_seconds))
    ...
    for candidate in _as_list(plan.get("entities")):
        ...
        for update in _as_list(candidate.get("semantic_field_updates")):
            ...
            if relationship == "revise" and field_name in semantic_fields:
                ...
                semantic_fields[field_name] = _deep_open_patch(semantic_fields.get(field_name), value) if isinstance(value, dict) else deepcopy(value)
