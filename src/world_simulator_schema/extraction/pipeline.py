...
from .cache import TaskCache, atomic_json
from .sources import message_rounds, budget_batches
from .continuity import load_state, overlay_network
from .protocols import options_for
...
def validate_entity_content_plan(
    ...
    *,
    ...
    source_text = {item["ref"]: item.get("content", "") for item in legacy.event_source_messages(rounds) if item.get("role") == "assistant"}
    for entry in [*plan.get("entities", []), *plan.get("relations", [])]:
        for evidence in entry.get("event_link_evidence", []):
            if not legacy._evidence_quote_in_text(str(evidence.get("quote", "")), source_text.get(str(evidence.get("source_ref", "")), "")):
                errors.append(f"{entry.get('entity_key') or entry.get('participant_keys')} 的证据不能完整定位到 AI 原文；未写入资料")
...
def run_probe(args: argparse.Namespace) -> dict[str, Any]:
    profiles = json.loads(Path(args.profiles).read_text(encoding="utf-8")) if getattr(args, "profiles", None) else {}
    ...
    def connection(prefix: str) -> dict[str, str]:
        ...
        return {
            ...
            "api_key": api_key,
            **({"client_options": profiles[prefix]} if prefix in profiles else {}),
        }
    ...
    snapshot_input = json.loads(Path(args.snapshot).read_text(encoding="utf-8")) if getattr(args, "snapshot", None) else {}
    state = load_state(snapshot_input)
    checkpoint_path = None
    completed_batches = 0
    restored_batches = []
    if getattr(args, "cache_dir", None):
        configuration = {task: {key: value for key, value in config.items() if key != "api_key"} for task, config in connections.items()}
        for task, config in configuration.items():
            config["credential"] = hashlib.sha256(connections[task]["api_key"].encode()).hexdigest()
        signature = hashlib.sha256(json.dumps(["checkpoint-3", snapshot_input.get("fingerprint"), chat_jsonl.read_text(encoding="utf-8"), configuration, system_prompts, workflow_config, args.batch_size, args.batch_chars, args.map_thinking, args.event_thinking, args.create_thinking, args.update_thinking, args.relation_thinking, args.map_max_tokens, args.event_max_tokens, args.entity_max_tokens, args.relation_max_tokens], ensure_ascii=False, sort_keys=True).encode()).hexdigest()
        checkpoint_path = Path(args.cache_dir) / ("checkpoint-" + signature + ".json")
        if checkpoint_path.exists():
            try:
                resume = json.loads(checkpoint_path.read_text(encoding="utf-8"))
                state = resume["state"]
                completed_batches = resume["completed_batches"]
                restored_batches = resume.get("diagnostics", [])
            except (ValueError, KeyError, OSError):
                pass
    previous_projection = legacy.materialize_network(deepcopy(state))[0]
    cache = TaskCache(Path(args.cache_dir) if getattr(args, "cache_dir", None) else None, snapshot_input.get("fingerprint", ""))

    def run_task(**kwargs):
        return cache.run(legacy.run_model_task, **kwargs)
    ...
    use_sources = bool(getattr(args, "snapshot", None))
    source_rounds = message_rounds(chat_jsonl) if use_sources else legacy.iter_rounds(chat_jsonl)
    batches = iter(budget_batches(source_rounds, args.batch_size, getattr(args, "batch_chars", 24000)))
    processed_narratives = {key: str(value.get("content", "")) for key, value in state.get("source_records", {}).items()}
    ...
    for batch_number in range(1, args.batches + 1):
        ...
        if batch_number <= completed_batches:
            print(f"第 {batch_number} 批从已校验检查点恢复", flush=True)
            continue
        ...
        map_result = run_task(
            task="narrative_map",
            client_options=connections["narrative_map"].get("client_options"),
        ...
        def execute(name: str) -> dict[str, Any]:
            ...
            return run_task(
                task=name,
                client_options=task_connection.get("client_options"),
        ...
        def execute_group(names: list[str]) -> dict[str, dict[str, Any]]:
            ...
            connection_values = {
                (connections[name]["endpoint"], connections[name]["model"], connections[name]["api_key"], json.dumps(connections[name].get("client_options"), sort_keys=True), specs[name]["thinking_mode"])
                for name in names
            }
            ...
            endpoint, model, merged_api_key, _, _ = next(iter(connection_values))
            system_prompt = "\n\n".join(
            ...
            merged = run_task(
                ...
                client_options=connections[names[0]].get("client_options"),
        ...
        for stage_index, groups in enumerate(stage_groups):
            ...
            with ThreadPoolExecutor(max_workers=max(1, min(len(groups), args.stage2_workers))) as pool:
                ...
                for future in as_completed(futures):
                    ...
                    persist_record(run, output, api_keys)
            if any(not task.get("ok") for task in batch["tasks"].values()):
                break
        ...
        generated, _ = legacy.materialize_network(candidate_state)
        network, network_report = overlay_network(candidate_state, generated, previous_projection)
        ...
        if checkpoint_path:
            diagnostics = [{"batch_number": item["batch_number"], "status": "restored", "tasks": {name: {"ok": task.get("ok"), "warnings": task.get("warnings", [])} for name, task in item.get("tasks", {}).items()}, "script_warnings": item.get("script_warnings", [])} for item in run["batches"]]
            atomic_json(checkpoint_path, {"state": state, "completed_batches": batch_number, "diagnostics": diagnostics})
        ...
        persist_record(run, output, api_keys)
        print(
            f"  第 {batch_number} 批已校验并缓存，等待宿主统一保存；网络 {network_report.get('entity_count')} 项",
            flush=True,
        )

    generated, _ = legacy.materialize_network(state)
    final_network, final_report = overlay_network(state, generated, previous_projection)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--transport-attempt-limit", type=int, default=3)
    thinking_choices = ("default", "off", "minimal", "low", "medium", "high", "xhigh", "max")
